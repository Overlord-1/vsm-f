...
const Rank = ({ color, topRank, rank, name, score }) => {
  ...
  return (
    <div
      className={`plate bg-[${topRank?color:"#1E1F26"}] p-5 mt-5 w-[95%] mx-auto flex items-center justify-between rounded-[18px]  border-[${
        topRank ? "none" : "none" 
      } opacity-50]`}
    >
      <div className="flex justify-start gap-5 ml-5 sm:ml-10 items-center ">
        ...
        :<div className="rank w-8 h-8 rounded-[50%] flex justify-center items-center bg-black p-2">{rank}</div>
        }
        
        {topRank ? (
          <div className="name font-bold text-opacity-70 text-black">{name}</div>
        ) : (
          <div
            className="name 
